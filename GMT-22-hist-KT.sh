#!/bin/bash
# Purpose: Plot two histograms of the cross-section profiles (here: Kermadec Trench)
...
gmt psrose tableK.txt -i1,4 -R0/1/0/360 \
    -A7r -S1.0in \
    -Gthistle -Bx0.2g0.2 \
    -By30g30+l"Cross-section profiles of the Kermadec trench" \
...
gmt pshistogram tableK.txt -i4 -R-10000/6/0/20 -JX4.8i/2.4i -X3.6i \
    ...
    -BWSne+t"Histograms of the depths frequency distribution: Kermadec Trench"+gsnow1 -Glightsteelblue1 \
    -D+f7p,Times-Roman,black -L0.1p,dimgray -Z1 -W250 -N0+pred -N1+pblue -N2+pgreen \
    -O -K >> $ps
# Step-5. Add legend
gmt pslegend -R -J -Dx1.0/-3.0+w6.2c+o-1.0/0.5c \
...
EOF
# Step-6. Add GMT logo
gmt logo -Dx7.0/0.0+o-0.5c/-2.5c+w2c -O >> $ps
# Step-7. Convert to image file using GhostScript (portrait orientation, 720 dpi)
gmt psconvert Hist_KT.ps -A0.2c -E720 -P -Tj -Z
