...
gmt pshistogram tableVTs.txt -i4 -R-7000/6/0/20 -JX4.8i/2.4i -X3.6i \
    ...
    -UBL/8.5c/-1.8c -K > $ps
echo "-7500 20 B" | gmt pstext -R -J -F+jBR+f15p,black -Gfloralwhite -W0.5p -O -K >> $ps
# Step-5. Add legend
...
gmt pshistogram tableVTn.txt -i4 -R-7000/6/0/20 -JX4.8i/2.4i -X15.0c\
    -Bpxg1000a1000f100+l"Bathymetry (m)" \
    -Bpyg5a5f2.5+l"Frequency"+u" %" -Bsyg2.5 \
-BWSne+t"Histograms on bathymetric data: Vityaz (A) and Vanuatu (B) trenches "+gsnow1 -Glightsteelblue1 \
    -D+f7p,Times-Roman,black -L0.1p,dimgray -Z1 -W250 -N0+pred -N1+pblue -N2+pgreen \
    -O -K >> $ps
echo "-7500 20 A" | gmt pstext -R -J -F+jBR+f15p,black -Gfloralwhite -W0.5p -O -K >> $ps
# Step-6. Add GMT logo
gmt logo -Dx6.0/0.0+o-0.0c/-10.0c+w2c -O >> $ps
# Step-7. Convert to image file using GhostScript (portrait orientation, 720 dpi)
gmt psconvert Hist_VVT.ps -A0.5c -E720 -P -Tj -Z
