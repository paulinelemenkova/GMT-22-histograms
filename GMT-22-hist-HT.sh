...
gmt pshistogram tableHis.txt -i4 -R-3500/6/0/25 -JX4.8i/2.4i -X3.6i \
    ...
    -UBL/8.5c/-1.8c -K > $ps
echo "-3800 22 B" | gmt pstext -R -J -F+jBR+f15p,black -Gfloralwhite -W0.5p -O -K >> $ps
# Step-5. Add legend
...
# NORTH
# Plot histogram
gmt pshistogram tableHic.txt -i4 -R-4000/6/0/25 -JX4.8i/2.4i -Y7.5c\
    -Bpxg1000a1000f100+l"Bathymetry (m)" \
    -Bpyg5a5f2.5+l"Frequency"+u" %" -Bsyg2.5 \
-BWSne+t"Histograms on bathymetric data: Hikurangi trench, northern (A) and southern (B) segments"+gsnow1 -Glightsteelblue1 \
    -D+f7p,Times-Roman,black -L0.1p,dimgray -Z1 -W250 -N0+pred -N1+pblue -N2+pgreen \
    -O -K >> $ps
echo "-3800 22 A" | gmt pstext -R -J -F+jBR+f15p,black -Gfloralwhite -W0.5p -O -K >> $ps
# Step-6. Add GMT logo
gmt logo -Dx6.0/0.0+o-0.0c/-10.0c+w2c -O >> $ps
# Step-7. Convert to image file using GhostScript (portrait orientation, 720 dpi)
gmt psconvert Hist_HT.ps -A0.5c -E720 -P -Tj -Z
